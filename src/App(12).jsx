...
// Extracts a £ price for a given duration (in minutes) from a free-text Pricing string,
// e.g. "30min — free initial call · 60min — £80 consultation" + duration=60 -> 80
// Returns 0 for durations explicitly marked "free".
const extractPrice = (pricingText, durationMin) => {
  if (!pricingText || !durationMin) return 0;
  const segments = pricingText.split(/[·•|]/);
  const durStr = String(durationMin);
  for (const seg of segments) {
    if (seg.includes(durStr + "min") || seg.includes(durStr + " min")) {
      if (/free/i.test(seg)) return 0;
      const match = seg.match(/£\s?([\d.]+)/);
      if (match) return parseFloat(match[1]);
      return 0;
    }
  }
  // Fallback: if only one £ amount exists anywhere in the string, use it
  const anyMatch = pricingText.match(/£\s?([\d.]+)/);
  return anyMatch ? parseFloat(anyMatch[1]) : 0;
};
...
function Dashboard({ onLogout, T, isDark, toggleTheme }) {
  ...
  const priceOf = (r) => {
    ...
    return extractPrice(consultant?.pricing, Number(r.duration));
  };
  ...
  const weekRevenue  = weekRes.reduce((sum, r) => sum + priceOf(r), 0);
  const monthRevenue = monthRes.reduce((sum, r) => sum + priceOf(r), 0);
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(2,1fr)", gap: 10 }}>
                <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: "16px" }}>
                  <div style={{ fontSize: 12, color: T.muted, marginBottom: 6 }}>💷 This week's revenue</div>
                  <div style={{ fontSize: 28, fontWeight: 800, color: T.accentT, lineHeight: 1 }}>£{weekRevenue.toLocaleString()}</div>
                  <div style={{ fontSize: 11, color: T.muted2, marginTop: 4 }}>{weekRes.length} session{weekRes.length === 1 ? "" : "s"} (excl. cancelled)</div>
                </div>
                <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: "16px" }}>
                  <div style={{ fontSize: 12, color: T.muted, marginBottom: 6 }}>💷 This month's revenue</div>
                  <div style={{ fontSize: 28, fontWeight: 800, color: T.accentT, lineHeight: 1 }}>£{monthRevenue.toLocaleString()}</div>
                  <div style={{ fontSize: 11, color: T.muted2, marginTop: 4 }}>{monthRes.length} session{monthRes.length === 1 ? "" : "s"} (excl. cancelled)</div>
                </div>
              </div>
              <div style={{ fontSize: 11, color: T.muted3, marginTop: -8 }}>Estimated from each consultant's Pricing field and the duration of each booking. Free initial calls count as £0.</div>
                ...
                <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: "16px" }}>
                  ...
                  {busiestDay ? (
                    <>
                      ...
                      <div style={{ fontSize: 12, color: T.muted2, marginTop: 2 }}>{busiestDay[1]} booking{busiestDay[1] === 1 ? "" : "s"} on average</div>
                    </>
                  ) : (
