...
const WEBHOOK_URL = "https://n8n-production-aed0a.up.railway.app/webhook/SignalCraft-Tax-api";
const UPDATE_URL  = "https://n8n-production-aed0a.up.railway.app/webhook/SignalCraft-Tax-update";
const BLOCKED_URL = "https://n8n-production-aed0a.up.railway.app/webhook/SignalCraft-Tax-blocked";
const CONSULTANTS_URL = "https://n8n-production-aed0a.up.railway.app/webhook/SignalCraft-Tax-consultants";
const REBUILD_PROMPT_URL = "https://n8n-production-aed0a.up.railway.app/webhook/SignalCraft-tax-rebuild-advisers-prompt";
...
// Fetch with timeout + automatic retries.
// n8n Cloud workflows can cold-start slowly (2-5s+) after being idle, which can
// make a single fetch attempt fail or hang. This retries a couple of times with
// a short backoff and a generous per-attempt timeout before giving up.
const fetchWithRetry = async (url, options = {}, { retries = 2, timeoutMs = 15000, backoffMs = 1000 } = {}) => {
  let lastErr;
...
function Dashboard({ onLogout, T, isDark, toggleTheme }) {
  ...
  const [cancelReason, setCancelReason] = useState(null); // null = not cancelling; "" or text = reason draft, shown as a sub-step in the detail modal
  ...
  const fetchReservations = async () => {
    try {
      ...
      const res = await fetchWithRetry(WEBHOOK_URL);
  ...
  const fetchBlocked = async () => {
    try { 
      const res = await fetchWithRetry(BLOCKED_URL); 
      const data = await res.json(); 
  ...
  const fetchConsultants = async () => {
    try {
      ...
      const res = await fetchWithRetry(CONSULTANTS_URL);
      const data = await res.json();
  ...
  const triggerRebuildPrompt = async () => {
    try {
      await fetchWithRetry(REBUILD_PROMPT_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({}) }, { retries: 1 });
  ...
  const saveConsultant = async () => {
    ...
    try {
      ...
      const res = await fetchWithRetry(CONSULTANTS_URL, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }, { retries: 1 });
      const data = await res.json();
  ...
  const deleteConsultant = async (id, consultant_id) => {
    try {
      await fetchWithRetry(CONSULTANTS_URL, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, consultant_id }) }, { retries: 1 });
  ...
  const addBlock = async () => {
    ...
    try {
      ...
      const res = await fetchWithRetry(BLOCKED_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(blockForm) }, { retries: 1 });
      const data = await res.json();
  ...
  const removeBlock = async (id) => {
    try {
      await fetchWithRetry(BLOCKED_URL, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) }, { retries: 1 });
  ...
  const updateStatus = async (id, status, reason) => {
    try {
      ...
      setDetail(null); setCancelReason(null);
      const body = { id, status };
      if (status === "Cancelled" && reason) body.reason = reason;
      await fetchWithRetry(UPDATE_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }, { retries: 1 });
  ...
  const weekdayBreakdown = (() => {
    ...
    monthRes.forEach(r => {
      ...
      if (counts[wk] !== undefined) counts[wk] += 1;
    });
    return order.map(d => [d, counts[d]]);
  })();
  ...
  // only customers with repeat bookings are useful to call out; a full list of every
  // single customer doesn't scale once there are dozens of them in a month.
  ...
  return (
    <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", height: isMobile ? "auto" : "100vh", minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "system-ui,sans-serif", overflow: isMobile ? "visible" : "hidden" }}>
      ...
      {/* Detail Modal */}
      {detail && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => { setDetail(null); setCancelReason(null); }}>
            ...
            {cancelReason === null ? (
              <>
                ...
                <div style={{ display: "flex", gap: 12, marginTop: 20 }}>
                  ...
                  <button onClick={() => setCancelReason("")} style={{ flex: 1, background: SC.Cancelled.bg, color: SC.Cancelled.text, border: `1px solid ${SC.Cancelled.border}`, borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 700, cursor: "pointer" }}>✗ Cancel</button>
                </div>
              </>
            ) : (
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <span style={{ fontWeight: 800, fontSize: 17, color: T.text }}>Cancel Booking — {detail.res_id}</span>
                  <button onClick={() => setCancelReason(null)} style={{ background: T.border, border: "none", color: T.muted, fontSize: 16, cursor: "pointer", borderRadius: 8, padding: "6px 10px" }}>✕</button>
                </div>
                <div style={{ fontSize: 13, color: T.muted, marginBottom: 14 }}>
                  The customer will receive an SMS letting them know their booking was cancelled, including the reason below.
                </div>
                <div style={{ fontSize: 12, color: T.muted2, marginBottom: 6 }}>Reason for cancellation</div>
                <textarea
                  autoFocus
                  value={cancelReason}
                  onChange={e => setCancelReason(e.target.value)}
                  placeholder="e.g. Adviser unavailable, customer requested another time, duplicate booking..."
                  style={{ width: "100%", minHeight: 100, background: T.input, border: `1px solid ${T.border2}`, borderRadius: 10, padding: "12px 14px", color: T.text, fontSize: 14, fontFamily: "inherit", resize: "vertical", boxSizing: "border-box" }}
                />
                <div style={{ display: "flex", gap: 12, marginTop: 20 }}>
                  <button onClick={() => setCancelReason(null)} style={{ flex: 1, background: T.border, color: T.muted, border: "none", borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 700, cursor: "pointer" }}>Back</button>
                  <button onClick={() => updateStatus(detail.id, "Cancelled", cancelReason.trim() || "Not specified")} style={{ flex: 1, background: SC.Cancelled.bg, color: SC.Cancelled.text, border: `1px solid ${SC.Cancelled.border}`, borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 700, cursor: "pointer" }}>✗ Confirm Cancellation</button>
                </div>
              </>
            )}
          </div>
        ...
        <div style={{ flex: isMobile ? undefined : 1, overflow: isMobile ? "visible" : "auto", padding: isMobile ? "12px" : "20px", display: "flex", flexDirection: "column", gap: isMobile ? 10 : 16 }}>
          ...
          {view === "insights" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, overflow: "hidden" }}>
                <div style={{ padding: "12px 16px", borderBottom: `1px solid ${T.border}`, fontWeight: 700, fontSize: 14, color: T.text }}>Bookings by day of week — this month</div>
                {monthRes.length === 0 ? (
                  <div style={{ padding: 32, textAlign: "center", color: T.muted3, fontSize: 14 }}>No bookings yet this month</div>
                ) : (
                  <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
                    {weekdayBreakdown.map(([name, count]) => {
                      const max = Math.max(1, ...weekdayBreakdown.map(([, c]) => c));
              ...
              {/* Top repeat customers this month */}
              <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, overflow: "hidden" }}>
                <div style={{ padding: "12px 16px", borderBottom: `1px solid ${T.border}`, fontWeight: 700, fontSize: 14, color: T.text }}>Top repeat customers — this month</div>
                {perCustomerMonthly.length === 0 ? (
                  <div style={{ padding: 32, textAlign: "center", color: T.muted3, fontSize: 14 }}>No repeat customers yet this month</div>
                ) : (
