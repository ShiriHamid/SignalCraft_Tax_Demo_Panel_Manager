...
function Dashboard({ onLogout, T, isDark, toggleTheme }) {
  ...
  const emptyConsultantForm = { consultant_id: "", name: "", department: "Mortgage Advice", title: "", location: "", direct_phone: "", direct_email: "", background: "", specialities: "", session_durations: "", pricing: "", best_for: "", note: "", working_hours: "Mon-Fri 09:00-17:00", calendar_email: "", active: true };
              ...
              <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 16 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 16 }}>
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="e.g. First-Time Buyer Mortgage Adviser" value={consultantForm.title} onChange={e => setConsultantForm(p => ({ ...p, title: e.target.value }))} style={inp} />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="e.g. 10 years experience, CeMAP qualified" value={consultantForm.background} onChange={e => setConsultantForm(p => ({ ...p, background: e.target.value }))} style={inp} />
                  </div>
                  <div>
                    <div style={{ fontSize: 12, color: T.muted2, marginBottom: 6 }}>Specialities</div>
                    <input type="text" placeholder="e.g. First-time buyer mortgages, Help to Buy" value={consultantForm.specialities} onChange={e => setConsultantForm(p => ({ ...p, specialities: e.target.value }))} style={inp} />
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <div>
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="e.g. 30min — free initial call · 60min full review" value={consultantForm.pricing} onChange={e => setConsultantForm(p => ({ ...p, pricing: e.target.value }))} style={inp} />
                  </div>
                  <div>
                    <div style={{ fontSize: 12, color: T.muted2, marginBottom: 6 }}>Best For</div>
                    <input type="text" placeholder="e.g. First-time buyers, new to the mortgage process" value={consultantForm.best_for} onChange={e => setConsultantForm(p => ({ ...p, best_for: e.target.value }))} style={inp} />
                  </div>
                  <div>
                    <div style={{ fontSize: 12, color: T.muted2, marginBottom: 6 }}>Note</div>
                    <input type="text" placeholder="e.g. Helped 300+ first-time buyers" value={consultantForm.note} onChange={e => setConsultantForm(p => ({ ...p, note: e.target.value }))} style={inp} />
                  </div>
                  <div>
